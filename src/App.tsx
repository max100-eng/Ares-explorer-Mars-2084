...
function App() {
  ...
  const preguntarAMarte = async () => {
    if (!pregunta) return;
    setCargando(true);
    setRespuesta("📡 Estableciendo enlace con el satélite orbital...");

    try {
      // ✅ CAMBIO CLAVE: Usamos 'gemini-pro' que es el modelo nativo de la v1
      ...
      setRespuesta(
        ...
        `Sugerencia: El modelo gemini-pro es el más estable para esta configuración.`
      );
    }
    setCargando(false);
  };
    ...
    <div className="min-h-screen bg-slate-900 text-white p-5 flex flex-col items-center justify-center font-sans">
      <header className="text-center mb-10">
        ...
        <p className="text-slate-400 uppercase tracking-widest text-xs mt-2">
          BASE MARTE 2084 | TRANSMISIÓN DE DATOS
        </p>
      </header>
      ...
      <main className="w-full max-w-xl bg-slate-800/50 backdrop-blur-md p-8 rounded-3xl border border-slate-700 shadow-2xl">
        <div className="mb-6">
          ...
          <textarea
            ...
            placeholder="Ej: Informe de integridad del casco en el hangar 4..."
        ...
        >
          {cargando ? "RECIBIENDO SEÑAL..." : "SOLICITAR TRANSMISIÓN 🚀"}
        </button>

        {respuesta && (
          <section className="mt-8 bg-black/40 p-6 rounded-2xl border-l-4 border-orange-500">
            <h3 className="text-[10px] font-bold text-orange-500 uppercase mb-2 tracking-widest">Respuesta de la Base:</h3>
      ...
      <footer className="mt-10 text-slate-600 text-[10px] tracking-widest uppercase">
        Terminal Status: Online | Protocol: Gemini-Pro Stable
      </footer>
    </div>
  );
}
